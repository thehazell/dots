...
-- play/pause spotify using playerctl
-- i have no idea if this works for other music apps or how it works, so youll have to adjust this if you dont use spotify
...
hl.bind(mainMod .. " + SHIFT + RETURN", hl.dsp.exec_cmd("hyprshutdown -p 'systemctl poweroff'"))
...
hl.bind(
    ...
    hl.dsp.exec_cmd(
        "if wpctl get-volume @DEFAULT_AUDIO_SINK@ | grep -q MUTED; then wpctl set-volume @DEFAULT_AUDIO_SINK@ 0; else wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-; fi"
    ),
